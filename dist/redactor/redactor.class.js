...
class Redactor {
    constructor() {
        ...
        this.PHONE_REGEX = /\d{3}-\d{3}-\d{4}/g;
        this.EMAIL_REGEX = /[\w\.-]+@[\w\.-]+\.\w+/g;
        this.ADDRESS_REGEX = /(?:\d+ )?[a-zA-Z. ]+(?:,\s?[a-zA-Z. ]+)?, [A-Z]{2} \d{5}(-\d{4})?/g;
        this.SSN_REGEX = /\d{3}-\d{2}-\d{4}/g;
        this.LICENSE_REGEX = /[a-zA-Z]{2}-\d{4,8}/g;
        this.CC_REGEX = /\d{4}[- ]?\d{4}[- ]?\d{4}[- ]?\d{4}/g;
        this.PASSPORT_REGEX = /[A-Z]{1,2}\d{6,9}/g;
        this.IBAN_REGEX = /[A-Z]{2}\d{2}[A-Z0-9]{4}\d{7}([A-Z0-9]?){0,16}/g;
        this.INTL_PHONE_REGEX = /\+\d{1,4}[\s-]?\d{1,4}[\s-]?\d{1,9}/g;
        ...
        this.generators = {
            ...
            passport() {
                ...
                const digits = Math.floor(Math.random() * (999999999 - 100000) + 100000).toString();
